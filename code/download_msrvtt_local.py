"""
MSR-VTT Dataset Downloader and Manifest Builder (Local Version)
Run this on your local machine to preprocess data before syncing to Zaratan
"""
import os
import sys
import json
...
MSR_VTT_ROOT = Path(os.environ.get("MSR_VTT_ROOT", "./data/msr-vtt"))
OUT_ROOT = Path(os.environ.get("OUT_ROOT", "./data/processed"))
...
def main():
    ...
    if not annotation_file.exists():
        alt_locations = [
            ...
            Path("./data/msr-vtt/annotation/MSR_VTT.json"),
        ]
    ...
    if has_local_videos:
        print(f"Found local videos directory: {local_videos_dir}")
    else:
        print(f"⚠️  No local videos found at {local_videos_dir}")
        print("   Will attempt to download from URLs if available")
    ...
    with open(MANIFEST, "w") as mf:
        for video in tqdm(split_videos, desc=f"Processing {SPLIT}"):
            ...
            if not video_found:
                # Store relative path for syncing
                rel_path = f"raw/videos/{SPLIT}/{video_id}{TARGET_EXT}" if video_found else None
                rec = dict(
                    id=video_id,
                    url=video.get("url", ""),
                    caption=caption,
            ...
            # Store relative path for syncing
            rel_path = f"raw/videos/{SPLIT}/{video_id}{TARGET_EXT}"
            ...
            rec = dict(
                ...
                caption=caption,
                captions=captions,
                path=rel_path,  # Relative path for syncing
                split=SPLIT,
                category=video.get("category", ""),
                **meta,
                status=status
    ...
    print(f"\n📦 Ready to sync to Zaratan:")
    print(f"   rsync -av {OUT_ROOT}/ vvr2211@login.zaratan.umd.edu:~/scratch.msml612-fa25/data/")
